/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package client;

import com.sun.jersey.api.client.Client;
import com.sun.jersey.api.client.ClientResponse;
import com.sun.jersey.api.client.GenericType;
import com.sun.jersey.api.client.UniformInterfaceException;
import com.sun.jersey.api.client.WebResource;
import entity.Customer;
import java.util.ArrayList;
import java.util.List;

/**
 * Jersey REST client generated for REST resource:CustomerFacadeREST
 * [entity.customer]<br>
 *  USAGE:
 * <pre>
 *        CustomerClient client = new CustomerClient();
 *        Object response = client.XXX(...);
 *        // do whatever with response
 *        client.close();
 * </pre>
 *
 * @author Lei
 */
public class CustomerClient {
    private WebResource webResource;
    private Client client;
    private static final String BASE_URI = "http://localhost:8080/MP3_RS/webresources";

    public CustomerClient() {
        com.sun.jersey.api.client.config.ClientConfig config = new com.sun.jersey.api.client.config.DefaultClientConfig();
        client = Client.create(config);
        webResource = client.resource(BASE_URI).path("entity.customer");
    }

    public void remove(String id) throws UniformInterfaceException {
        webResource.path(java.text.MessageFormat.format("{0}", new Object[]{id})).delete();
    }

    public String countREST() throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path("count");
        return resource.accept(javax.ws.rs.core.MediaType.TEXT_PLAIN).get(String.class);
    }

    public <T> T findAll_XML(Class<T> responseType) throws UniformInterfaceException {
        WebResource resource = webResource;
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_XML).get(responseType);
    }

    public <T> T findAll_JSON(Class<T> responseType) throws UniformInterfaceException {
        WebResource resource = webResource;
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_JSON).get(responseType);
    }

    public void edit_XML(Object requestEntity) throws UniformInterfaceException {
        webResource.type(javax.ws.rs.core.MediaType.APPLICATION_XML).put(requestEntity);
    }

    public void edit_JSON(Object requestEntity) throws UniformInterfaceException {
        webResource.type(javax.ws.rs.core.MediaType.APPLICATION_JSON).put(requestEntity);
    }

    public void create_XML(Object requestEntity) throws UniformInterfaceException {
        webResource.type(javax.ws.rs.core.MediaType.APPLICATION_XML).post(requestEntity);
    }

    public void create_JSON(Object requestEntity) throws UniformInterfaceException {
        webResource.type(javax.ws.rs.core.MediaType.APPLICATION_JSON).post(requestEntity);
    }

    public <T> T findRange_XML(Class<T> responseType, String from, String to) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("{0}/{1}", new Object[]{from, to}));
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_XML).get(responseType);
    }

    public <T> T findRange_JSON(Class<T> responseType, String from, String to) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("{0}/{1}", new Object[]{from, to}));
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_JSON).get(responseType);
    }

    public <T> T find_XML(Class<T> responseType, String id) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("{0}", new Object[]{id}));
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_XML).get(responseType);
    }

    public <T> T find_JSON(Class<T> responseType, String id) throws UniformInterfaceException {
        WebResource resource = webResource;
        resource = resource.path(java.text.MessageFormat.format("{0}", new Object[]{id}));
        return resource.accept(javax.ws.rs.core.MediaType.APPLICATION_JSON).get(responseType);
    }

    public void close() {
        client.destroy();
    }
    
    public static List<Customer> getCustomerList() {
        List<Customer> customers = new ArrayList<Customer>();
        CustomerClient customerClient = new CustomerClient();
        ClientResponse clientResponse = customerClient.findAll_XML(ClientResponse.class);
        GenericType<List<Customer>> genericType = new GenericType<List<Customer>>() {
        };
        customers = clientResponse.getEntity(genericType);
        return customers;
    }

    public static void createCustomer(Customer c) {
        CustomerClient client = new CustomerClient();
        ClientResponse clientResponse = client.findAll_XML(ClientResponse.class);
        GenericType<List<Customer>> genericType = new GenericType<List<Customer>>() {
        };
        List<Customer> customers = new ArrayList<Customer>();
        customers = clientResponse.getEntity(genericType);
        Customer customer = new Customer();
        customer.setCustomerId(c.getCustomerId());
        customer.setDiscountCode('H');
        customer.setZip("10095");
        customer.setName(c.getName());
        customer.setAddressline1(c.getAddressline1());
        customer.setAddressline2(c.getAddressline2());
        customer.setCity(c.getCity());
        customer.setState(c.getState());
        customer.setPhone(c.getPhone());
        customer.setFax(c.getFax());
        customer.setEmail(c.getEmail());
        customer.setCreditLimit(c.getCreditLimit());
        client.create_XML(customer);
    }

    public static void updateCustomer(Customer c) {
        CustomerClient client = new CustomerClient();
        int id = c.getCustomerId();
        /**
         * List<Customer> customers = getCustomerList(); for (int i = 0 ; i <
         * customers.size(); i++){ if (i == id){ id =
         * customers.get(i).getCustomerId(); break; } }*
         */
        String s = String.valueOf(id);
        ClientResponse clientResponse = client.find_XML(ClientResponse.class, s);
        GenericType<Customer> genericType = new GenericType<Customer>() {
        };
        Customer customer = (Customer) clientResponse.getEntity(genericType);
        customer.setName(c.getName());
        customer.setCustomerId(c.getCustomerId());
        customer.setDiscountCode('H');
        customer.setZip("10095");
        customer.setAddressline1(c.getAddressline1());
        customer.setAddressline2(c.getAddressline2());
        customer.setCity(c.getCity());
        customer.setState(c.getState());
        customer.setPhone(c.getPhone());
        customer.setFax(c.getFax());
        customer.setEmail(c.getEmail());
        customer.setCreditLimit(c.getCreditLimit());
        client.edit_XML(customer);
    }

    public static Customer retrieveCustomer(String num) {
        CustomerClient client = new CustomerClient();
        int n = Integer.valueOf(num);
        List<Customer> customers = getCustomerList();
        for (int i = 0; i < customers.size(); i++) {
            if (i+1== n) {
                n = customers.get(i).getCustomerId();
                break;
            }
        }
        String s = String.valueOf(n);
        ClientResponse clientResponse = client.find_XML(ClientResponse.class, s);
        GenericType<Customer> genericType = new GenericType<Customer>() {
        };
        Customer customer = (Customer) clientResponse.getEntity(genericType);
        return customer;
    }
    
    public static void deleteCustomer(String num){
         CustomerClient client = new CustomerClient();
        int n = Integer.valueOf(num);
        List<Customer> customers = getCustomerList();
        for (int i = 0; i < customers.size(); i++) {
            if (i+1== n) {
                n = customers.get(i).getCustomerId();
                break;
            }
        }
        String s = String.valueOf(n);
        client.remove(s);
    }
}
